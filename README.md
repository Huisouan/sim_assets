...
- Throughout the repository, the name `sim_assets` only serves as an example and we provide a script to rename all the references to it automatically:

```bash
# Enter the repository
cd IsaacLabExtensionTemplate
# Rename all occurrences of sim_assets (in files/directories) to your_fancy_extension_name
...
We provide an example UI extension that will load upon enabling your extension defined in `source/sim_assets/sim_assets/ui_extension_example.py`.
...
```json
{
    "python.analysis.extraPaths": [
        "<path-to-ext-repo>/source/sim_assets"
    ]
}
```
